...
This section here is intended for a single Docker container setup for the data pipeline and RAG.
...
This section will outline the necessary steps and pre-requisites to launch the Web Application.

### Pre-requisites

- [**Node.js**](https://docs.npmjs.com/downloading-and-installing-node-js-and-npm)
  - Required to set up our development environment.
  - Note that we will also require the installation of `npm`, which is the package manager that we will use for our set-up.
- [**Docker Desktop**](https://www.docker.com/products/docker-desktop/)
  - Needed to run containers
  - In our case, we will require it to run our MongoDB and PostgreSQL services for our application.

> [!NOTE]
> The installation of Node.js should be rather straightforward. For troubleshooting purposes, you may also refer to the troubleshooting section in the link above.
> The installation of Docker Desktop should be straightforward just by clicking the link and following the official Download instructions.

### Workflow

1. **Clone the Repository** (Can be skip to step 2 if already cloned)
...
2. **Set-up Repo environment variables in Backend**
   Open up the repo in IDE (VSCode) and navigate to the `/backend` folder. Then, create a `.env` file in the following folder and set the following environment variables:

- `MONGODB_USERNAME`: Username of user accessing our MongoDB service using Docker.
- `MONGODB_PASSWORD`: Password of user accessing our MongoDB service using Docker.
...
> [!NOTE]
> For the MongoDB user, we will need to do further set-up to create our root user if not done so previously.
> This will be covered in step 4.

3. **Initialization of Backend Server**
   With the environment variables set up, we will now try to run our backend server.
   Firstly, ensure that your `Docker Desktop` is running in the background.
   Afterwards, in the `/backend` folder directory, run the following command:
   ```shell
   docker compose up -d
   ```
   This will run the `docker-compose.yaml` in the directory, which sets up the MongoDB and PostgreSQL services for you.
   It will automatically pull the `mongodb` and `postgres` images from Docker if it is not already in your system, and then create running containers for both services.
4. **Setting up of MongoDB user**
...
> [!IMPORTANT]
> Again, it is crucial that the details used in creating the user must be the same as the ones input in the `.env` file.
> If not, there will be errors when connecting to the MongoDB server using JavaScript's `mongoose` library.

5. **Installing of Backend Libraries**
   Once the above is done, we will now need to run `npm install` in the `/backend` folder to install all the necessary packages.
...
This command will run the `start` script, which runs both the `docker compose up -d` and `node src/server.js` commands. This will set up the necessary services for our application to work.

> [!NOTE]
> The scripts can be found in the `package.json` file in the `/backend` folder.

7. **Installing of Frontend Libraries**
   Now that our backend is connected, we will now need to launch our frontend.
   You may create another terminal and change into the frontend folder from the root directory as such:

   ```shell
   cd frontend
   ```

   In this directory, we will then install all our Frontend packages using the command:
